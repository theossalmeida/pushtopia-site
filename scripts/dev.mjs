...
const publicFiles = new Set(["/", "/index.html", "/styles.css", "/badges.css", "/script.js", "/config.js"]);
const server = createServer(async (request, response) => {
  const requested = new URL(request.url || "/", "http://localhost").pathname;
  if (!publicFiles.has(requested) && !requested.startsWith("/assets/sprites/")) { response.writeHead(404); response.end("Not found"); return; }
